...
def calc_adx(highs, lows, closes, period=14):
    ...
    plus_dm = np.array(plus_dm[-period*2:])
    minus_dm = np.array(minus_dm[-period*2:])
    tr = np.array([max(highs[i]-lows[i], abs(highs[i]-closes[i-1]), abs(lows[i]-closes[i-1])) for i in range(1, len(closes))])[-period*2:]
    
    plus_di = np.mean(plus_dm) / (np.mean(tr) + 0.0001) * 100
    minus_di = np.mean(minus_dm) / (np.mean(tr) + 0.0001) * 100
    
    dx = abs(plus_di - minus_di) / (plus_di + minus_di + 0.0001) * 100
    adx = np.mean(dx[-period:]) if len(dx) >= period else dx.mean()
    return adx

def detect_market_regime_v8(closes_3m, highs_3m, lows_3m, closes_1h=None, highs_1h=None, lows_1h=None):
    """
    v8市场状态探测 - 三维度综合判断
    返回: 'grid' (网格/横盘), 'momentum' (趋势), 'mixed' (混合)
    """
    try:
        if len(closes_3m) < 60:
            return 'mixed'
        ...
        # 维度1: ADX趋势强度
        adx = calc_adx(highs_3m, lows_3m, closes_3m, 14)
        
        # 维度2: 波动率比 (短期/长期)
        vol_short = float(np.std(np.diff(closes_3m[-VOL_SHORT:]) / closes_3m[-VOL_SHORT:-1]) * 100) if len(closes_3m) > VOL_SHORT else 1
        vol_long = float(np.std(np.diff(closes_3m[-VOL_LONG:]) / closes_3m[-VOL_LONG:-1]) * 100) if len(closes_3m) > VOL_LONG else 1
        vol_ratio = vol_short / (vol_long + 0.0001)
        
        # 维度3: RSI稳定性 (横盘时RSI在40-60波动小)
        rsi_vals = [calc_rsi(closes_3m[:i+1]) for i in range(len(closes_3m)-14, len(closes_3m))]
        rsi_std = float(np.std(rsi_vals)) if len(rsi_vals) > 1 else 0
        rsi_mean = float(np.mean(rsi_vals)) if len(rsi_vals) > 0 else 50
        rsi_stable = 40 < rsi_mean < 60 and rsi_std < RSI_STABLE_RANGE
        
        # 综合判断
        # 网格/横盘: ADX低 + 波动率稳定 + RSI稳定
        # 趋势: ADX高 + 明显的方向性
        # 混合: 其他情况
        ...
        if adx < ADX_THRESHOLD: grid_score += 2
        elif adx > 30: momentum_score += 2
        
        if 0.7 < vol_ratio < 1.3: grid_score += 1
        elif vol_ratio > 1.5: momentum_score += 1
        
        if rsi_stable: grid_score += 1
        
        if grid_score >= 3: return 'grid'
        elif momentum_score >= 2: return 'momentum'
        else: return 'mixed'
        
    except Exception as e:
        return 'mixed'

def grid_trade_logic(sym, price, position_data, prices, all_holdings, grid_state):
    """
    网格交易逻辑 - 专门处理横盘震荡
    返回: (action, qty) 或 (None, 0)
    action: 'grid_buy', 'grid_sell', 'grid_tp', None
    """
    if not GRID_MODE_ENABLED:
        return None, 0
    ...
    try:
        ...
        if sym not in grid_state:
            ...
            for i in range(GRID_LEVELS):
                level_price = price * (1 - GRID_SPACING * (i + 1))
                grid_state[sym]['grid_levels'][i] = {
                    'buy_price': level_price,
                    'sell_price': level_price * (1 + GRID_SPACING * GRID_TP_MULTIPLE),
                    'active': False,
                    'qty': 0
                }
            return None, 0
        ...
        # 检查是否可以操作 (冷却)
        if time.time() - gs['last_action_time'] < GRID_COOLDOWN:
            return None, 0
        
        # 计算当前价格在哪一格
        current_level = -1
        for i in range(GRID_LEVELS):
            lp = gs['grid_levels'][i]['buy_price']
            if current_price <= lp * (1 + GRID_SPACING * 0.5):
                current_level = i
                break
        
        # 网格买入逻辑 - 价格下跌到某格时买入
        if current_level >= 0 and gs['active_layers'] < GRID_MAX_EXPOSURE * 100:
            level_info = gs['grid_levels'][current_level]
            if not level_info['active'] and current_price <= level_info['buy_price']:
                # 买入这一格
                qty = budget * GRID_POSITION_SIZE / current_price
                qty = round_to_step(qty, get_exchange_info(sym).get('stepSize', 0.1))
                level_info['active'] = True
                level_info['qty'] = qty
                gs['active_layers'] += 1
                gs['last_action'] = 'buy'
                gs['last_action_time'] = time.time()
                log(f'  📊 {sym} 网格买入 L{current_level} @ ${current_price:.4f}')
                return 'grid_buy', qty
        
        # 网格止盈逻辑 - 价格回升到止盈位时卖出
        for i in range(GRID_LEVELS):
            level_info = gs['grid_levels'][i]
            if level_info['active'] and current_price >= level_info['sell_price']:
                qty = level_info['qty']
                ...
                gs['last_action'] = 'sell'
                gs['last_action_time'] = time.time()
                profit_pct = (current_price - level_info['buy_price']) / level_info['buy_price'] * 100
                log(f'  📊 {sym} 网格止盈 L{i} @ ${current_price:.4f} (+{profit_pct:.2f}%)')
                return 'grid_tp', qty
        
        return None, 0
        
    except Exception as e:
        log(f'  ⚠️ {sym} 网格异常: {str(e)[:50]}')
        return None, 0
...
def main():
    ...
    grid_state = {}  # v8: 网格状态跟踪
    market_mode = "momentum"  # v8: 'grid', 'momentum', 'mixed'
    last_market_check = 0  # v8: 上次市场检测时间
    ...
    while running:
        try:
            ...
            for sym, data in sorted(all_holdings.items(), key=lambda x: -x[1]['value']):
                ...
                entry_data = position_entries.get(sym, {'time': time.time(), 'price': data.get('price', 0)})
                entry_time = entry_data.get('time', time.time())
                entry_price = entry_data.get('price', data.get('price', 0))
            ...
            if market_mode == 'grid':
                for sym in prices.keys():
                    ...
                    if sym not in all_holdings or all_holdings[sym].get('value', 0) < MIN_VALUE:
                        action, qty = grid_trade_logic(sym, prices[sym], None, prices, all_holdings, grid_state)
                        if action == 'grid_buy' and qty > 0:
                            grid_buys.append({'sym': sym, 'qty': qty, 'price': prices[sym], 'source': 'grid'})
                        elif action == 'grid_tp' and qty > 0:
                            # 网格止盈卖出
                            info = get_exchange_info(sym)
                            free = all_holdings.get(sym, {}).get('amount', 0)
                            step_size = info.get('stepSize', 0.1)
                            qty_to_sell = round_to_step(min(qty, free), step_size)
                            if qty_to_sell >= info.get('minQty', 0.1) and qty_to_sell * prices[sym] >= info.get('minNotional', 5):
                                if place_order(sym, 'SELL', qty_to_sell, prices[sym]).get('success'):
                                    sell_count += 1; trades += 1
                                    log(f'  📊 {sym} 网格止盈完成')
            
            # v8: 趋势检测 - 如果BTC在下跌趋势中，减少买入
            btc_trend = "unknown"
            try:
                btc_closes = np.array([x[0] for x in get_klines('BTC', '1h', 50)])
                if len(btc_closes) >= 20:
                    btc_ma5 = np.mean(btc_closes[-5:])
                    btc_ma20 = np.mean(btc_closes[-20:])
                    btc_rsi = calc_rsi(btc_closes)
                    if btc_closes[-1] < btc_ma5 < btc_ma20 and btc_rsi < 45:
                        btc_trend = "down"
                    elif btc_closes[-1] > btc_ma5 > btc_ma20:
                        btc_trend = "up"
                    else:
                        btc_trend = "neutral"
            except:
            ...
            # v8: 混合买入列表 - 网格优先，然后是信号
            # 下跌趋势中只做网格交易，不买信号
            all_buys = grid_buys.copy()
            if btc_trend != "down":  # 非下跌趋势才买信号
                for d in strong_buys[:4]:
                    all_buys.append({'sym': d['sym'], 'qty': None, 'price': d['price'], 'signal': d['signal'], 'source': 'signal'})
                ...
                log('  📥 {} [{}] RSI:{:.0f} BB:{:.2f} \${:.2f}'.format(sym, d.get('source','signal'), d.get('rsi',0), d.get('bb',0), order_value))
                if place_order(sym, 'BUY', qty, price).get('success'):
                    buy_count += 1; trades += 1; spot_usdt -= budget; open_pos += 1
                    position_entries[sym] = {'time': time.time(), 'price': price}
