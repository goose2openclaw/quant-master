...
COOLDOWN = 15           # 15秒冷却
BLACKLIST = ['NEIRO', 'TURBO', 'PEPE', 'FLOKI', 'BOME', 'SHIB', 'DOGE', 'VR', 'VRQQ']
    ...
    while running:
        try:
            ...
            for sym in prices.keys():
                ...
                sig = 0
                # 超卖信号
                if rsi < 35 and bb_pos < 0.3: sig = 0.4
                elif rsi < 40 and bb_pos < 0.25: sig = 0.3
                elif momentum > 0.003 and bb_pos < 0.4: sig = 0.25
                # 超买信号
                if rsi > 65 or bb_pos > 0.8: sig = -0.3
                
                if sig > BUY_SIGNAL_T: strong_buys.append({'sym': sym, 'signal': sig, 'price': prices[sym], 'rsi': rsi, 'bb': bb_pos})
