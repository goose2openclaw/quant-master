...
LOG_FILE = '/home/goose/.openclaw/workspace/logs/g44_v44.log'
    ...
    for retry in range(3):
        try:
            ...
            if not data or len(data) < 50:
                log('  {} K线数据不足: {}'.format(sym, len(data) if data else 0))
                return []
            return data
        except Exception as e:
            log('  {} K线获取失败[{}]: {}'.format(sym, retry+1, str(e)[:30]))
            time.sleep(1)
    return []
...
def main():
    global running
    def signal_handler(s, f): log('G44 v4.4停止...'); global running; running = False
    sig_module.signal(sig_module.SIGTERM, signal_handler)
    sig_module.signal(sig_module.SIGINT, signal_handler)
    cycle = 0; trades = 0; errors = 0
    log('=' * 70)
    log('G44 v4.4 修复版启动')
    log('修复: list index out of range错误')
    log('=' * 70)
    while running:
        try:
            cycle += 1
    ...
    log('G44 v4.4停止 - 周期{} 交易{} 错误{}'.format(cycle, trades, errors))
