...
LOG_FILE = '/home/goose/.openclaw/workspace/logs/g44_live.log'
        ...
        try:
            ...
            try:
                ...
                if 'LOT_SIZE' in err_msg or 'MIN_NOTIONAL' in err_msg:
                    qty *= 1.5; log('  LOT_SIZE修复: x1.5'); time.sleep(0.3); continue
...
def main():
    global running
    def signal_handler(s, f): log('G44停止...'); global running; running = False
    sig_module.signal(sig_module.SIGTERM, signal_handler)
    sig_module.signal(sig_module.SIGINT, signal_handler)
    cycle = 0; trades = 0; errors = 0
    log('=' * 70)
    log('G44 v4.1 Live 实盘自动交易启动')
    log('自主杠杆 | 自动纠错 | 资产调配 | 利益最大化')
    log('=' * 70)
    while running:
        try:
            cycle += 1
            ...
            for sym, h in holdings.items():
                ...
                lev = d['leverage']
                if c < SELL_T or pnl < -4 * lev:
                    val = h['amount'] * h['price']
                    if val >= 3:
                        sells.append({'sym': sym, 'amt': h['amount']*0.9, 'c': c, 'pnl': pnl, 'lev': lev})
