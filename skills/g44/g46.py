...
LOG_FILE = '/home/goose/.openclaw/workspace/logs/g46.log'
...
def check_tp_sl(current_price, avg_cost):
...
def main():
    global running
    def signal_handler(s, f): log('G46 停止...'); global running; running = False
    sig_module.signal(sig_module.SIGTERM, signal_handler)
    sig_module.signal(sig_module.SIGINT, signal_handler)
    cycle = 0; trades = 0; tp_count = 0; sl_count = 0; buy_count = 0; sell_count = 0
    log('=' * 70)
    log('G46 v15 - 现货+全仓管家(NOTIONAL精确版)')
    ...
    while running:
        try:
            ...
            log('')
            log('=== G46周期{} | 总资产:\${:.2f} | 现货USDT:\${:.2f} | 全仓:\${:.2f} ==='.format(cycle, total_asset, spot_usdt, cross_total))
            if all_holdings:
            ...
            for sym, data in sorted(all_holdings.items(), key=lambda x: -x[1]['value']):
                ...
                action, pnl_pct = check_tp_sl(current, avg_cost)
                if action == 'TP':
            ...
            for d in strong_buys[:2]:
                ...
                # 计算满足min_notional的最小数量
                qty = max(budget / price, min_notional / price)
                qty = round_up_to_step(qty, step_size)  # 向上取整确保满足min_notional
                order_value = qty * price
            ...
            if cycle % 5 == 0:
                log('统计: 周期{} 交易{} 买{} 卖{} 止盈{} 止损{}'.format(cycle, trades, buy_count, sell_count, tp_count, sl_count))
            for _ in range(60):
                if not running: break
                time.sleep(1)
        except Exception as e:
            log('异常: ' + str(e)[:80]); time.sleep(5)
    log('G46停止 - 周期{} 交易{} 买{} 卖{} 止盈{} 止损{}'.format(cycle, trades, buy_count, sell_count, tp_count, sl_count))
