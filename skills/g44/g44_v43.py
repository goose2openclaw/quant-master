...
LOG_FILE = '/home/goose/.openclaw/workspace/logs/g44_v43.log'
...
def main():
    global running
    def signal_handler(s, f): log('G44 v4.3停止...'); global running; running = False
    sig_module.signal(sig_module.SIGTERM, signal_handler)
    sig_module.signal(sig_module.SIGINT, signal_handler)
    cycle = 0; trades = 0; errors = 0
    log('=' * 70)
    log('G44 v4.3 自主决策实施版启动')
    log('精度修复 | 自主决策 | 自动实施')
    log('=' * 70)
    while running:
        try:
    ...
    log('G44 v4.3停止 - 周期{} 交易{} 错误{}'.format(cycle, trades, errors))
