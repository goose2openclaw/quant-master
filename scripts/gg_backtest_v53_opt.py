...
class Backtester:
    ...
    def default_cfg(self, mode):
        if mode == 'normal':
            return {
                'leverage': 1.0, 'position_size': 0.1,
                'buy_th': 20, 'sell_th': 80,
                'stop_loss': 0.05, 'take_profit': 0.10,
                'position_ratio': 0.3, 'rebalance_th': 0.1,
            }
        return {
            'leverage': 2.0, 'position_size': 0.2,
            'buy_th': 18, 'sell_th': 82,
            'stop_loss': 0.02, 'take_profit': 0.12,
            'position_ratio': 0.5, 'rebalance_th': 0.08,
        }
    ...
    def simulate(self, price_data):
        for day_idx in range(len(price_data[COINS[0]])):
            ...
            for c in COINS:
                ...
                # 再平衡
                target = total_value * self.cfg['position_size']
                diff = current_pos_value - target
                if abs(diff) / total_value > self.cfg['rebalance_th']:
                    if diff > 0:
                        qty = diff / d['close'] * 0.5
                        if qty <= self.positions[c]:
                            self.capital += qty * d['close'] * 0.999
                            self.positions[c] -= qty
                            self.trades.append({'type':'REBAL','coin':c,'price':d['close'],'day':day_idx})
                    else:
                        qty = -diff / d['close'] * 0.5
                        cost = qty * d['close'] * 1.001
                        if cost <= self.capital:
    ...
    def get_stats(self, final_value):
        ...
        wins = losses = 0
        for sell in sells:
            prev = [b for b in reversed(buys) if b['coin'] == sell['coin'] and b['day'] < sell['day']]
            if prev and sell['price'] > prev[0]['price']:
                wins += 1
            else:
                losses += 1
        ...
        return {
            ...
            'leverage': self.cfg['leverage'], 'position_size': self.cfg['position_size'],
            'capital_usage_avg': avg_used, 'capital_usage_max': max_used,
            'total_trades': len(self.trades), 'buys': len(buys), 'sells': len(sells),
            'wins': wins, 'losses': losses
        }

def main():
    print("="*70)
    print("Hermes v5.3 Optimized - 30天回测")
    print("普通模式 vs 专家模式")
    print("="*70)
    ...
    configs = [
        # 普通模式
        {'mode':'normal','leverage':1.0,'position_size':0.1,'buy_th':20,'sell_th':80,'position_ratio':0.3,'rebalance_th':0.1},
        {'mode':'normal','leverage':1.5,'position_size':0.15,'buy_th':20,'sell_th':80,'position_ratio':0.35,'rebalance_th':0.1},
        {'mode':'normal','leverage':2.0,'position_size':0.15,'buy_th':18,'sell_th':82,'position_ratio':0.4,'rebalance_th':0.08},
        {'mode':'normal','leverage':2.0,'position_size':0.2,'buy_th':18,'sell_th':82,'position_ratio':0.4,'rebalance_th':0.1},
        {'mode':'normal','leverage':3.0,'position_size':0.1,'buy_th':20,'sell_th':80,'position_ratio':0.3,'rebalance_th':0.1},
        # 专家模式
        {'mode':'expert','leverage':2.0,'position_size':0.2,'buy_th':18,'sell_th':82,'position_ratio':0.5,'rebalance_th':0.08},
        {'mode':'expert','leverage':2.0,'position_size':0.25,'buy_th':15,'sell_th':85,'position_ratio':0.5,'rebalance_th':0.08},
        {'mode':'expert','leverage':3.0,'position_size':0.2,'buy_th':18,'sell_th':82,'position_ratio':0.45,'rebalance_th':0.08},
        {'mode':'expert','leverage':3.0,'position_size':0.25,'buy_th':15,'sell_th':85,'position_ratio':0.5,'rebalance_th':0.08},
        {'mode':'expert','leverage':5.0,'position_size':0.15,'buy_th':18,'sell_th':82,'position_ratio':0.4,'rebalance_th':0.1},
        {'mode':'expert','leverage':5.0,'position_size':0.2,'buy_th':15,'sell_th':85,'position_ratio':0.5,'rebalance_th':0.08},
    ]
    ...
    print("【3. 回测结果矩阵 - 普通模式】")
    print("="*70)
    print(f"\n{'配置':14} {'杠杆':5} {'仓位':5} {'收益':9} {'胜率':7} {'资金(均/最大)':16} {'交易':6} {'胜负':8}")
    print("-"*70)
    for r in normal:
        cfg_str = f"买{r['buy_th']}/卖{r['sell_th']}"
        print(f"{cfg_str:14} {r['leverage']:>4.1f}x {r['position_ratio']*100:>4.0f}% {r['return']:>+8.2f}% {r['win_rate']:>5.1f}% {r['capital_usage_avg']:>5.1f}%/{r['capital_usage_max']:>5.1f}% {r['total_trades']:>5d} {r['wins']:>3d}/{r['losses']:<3d}")
...
""")
    ...
    with open('/tmp/backtest_v53_opt.json', 'w') as f:
