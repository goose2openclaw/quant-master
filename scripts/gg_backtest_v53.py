...
class Backtester:
    ...
    def simulate(self, price_data):
        for day_idx in range(len(price_data[COINS[0]])):
            ...
            self.history.append({'day':day_idx,'portfolio':total_value,'used_ratio':used_ratio,'cash':self.capital})
            ...
            for c in COINS:
                ...
                # 买入信号 - 低吸
                if pos < self.cfg['buy_th'] and self.capital > 10:
                    ...
                    if cost <= self.capital:
                        ...
                        self.trades.append({'type':'BUY','coin':c,'price':d['close'],'day':day_idx,'pos':pos,'account':'SPOT'})
                ...
                # 卖出信号 - 高抛
                    ...
                    self.positions[c] -= qty
                    self.trades.append({'type':'SELL','coin':c,'price':d['close'],'day':day_idx,'pos':pos,'account':'SPOT'})
                
                # 再平衡信号 - v53特有
                target_pos_value = total_value * self.cfg['position_size']
                if abs(current_pos_value - target_pos_value) / total_value > self.cfg['rebalance_threshold']:
                    if current_pos_value < target_pos_value and self.capital > 10:
                        diff = target_pos_value - current_pos_value
                        qty = diff / d['close'] * 0.5
                        cost = qty * d['close'] * 1.001
                        if cost <= self.capital:
                            self.capital -= cost
                            self.positions[c] += qty
                            self.trades.append({'type':'REBALANCE_BUY','coin':c,'price':d['close'],'day':day_idx})
                    elif current_pos_value > target_pos_value:
                        diff = current_pos_value - target_pos_value
                        qty = diff / d['close'] * 0.5
                        if qty <= self.positions[c]:
                            revenue = qty * d['close'] * 0.999
                            self.capital += revenue
                            self.positions[c] -= qty
                            self.trades.append({'type':'REBALANCE_SELL','coin':c,'price':d['close'],'day':day_idx})
    ...
    def get_stats(self, final_value):
        ...
        sells = [t for t in self.trades if t['type'] in ['SELL','REBALANCE_SELL']]
        buys = [t for t in self.trades if t['type'] in ['BUY','REBALANCE_BUY']]
...
def main():
    print("="*80)
    print("Hermes v5.3 (蒸馏前版本) - 30天全面回测")
    print("普通模式 vs 专家模式")
    print("="*80)
    ...
    configs = [
        # 普通模式
        {'mode':'normal','leverage':1.0,'position_size':0.1,'buy_th':20,'sell_th':80,'position_ratio':0.3,'rebalance_threshold':0.1},
        {'mode':'normal','leverage':1.0,'position_size':0.15,'buy_th':20,'sell_th':80,'position_ratio':0.4,'rebalance_threshold':0.1},
        {'mode':'normal','leverage':1.5,'position_size':0.1,'buy_th':20,'sell_th':80,'position_ratio':0.3,'rebalance_threshold':0.1},
        {'mode':'normal','leverage':2.0,'position_size':0.1,'buy_th':15,'sell_th':85,'position_ratio':0.3,'rebalance_threshold':0.15},
        {'mode':'normal','leverage':2.0,'position_size':0.15,'buy_th':18,'sell_th':82,'position_ratio':0.4,'rebalance_threshold':0.12},
        {'mode':'normal','leverage':3.0,'position_size':0.1,'buy_th':20,'sell_th':80,'position_ratio':0.2,'rebalance_threshold':0.1},
        # 专家模式
        {'mode':'expert','leverage':2.0,'position_size':0.2,'buy_th':15,'sell_th':85,'position_ratio':0.5,'rebalance_threshold':0.15},
        {'mode':'expert','leverage':2.0,'position_size':0.25,'buy_th':15,'sell_th':85,'position_ratio':0.5,'rebalance_threshold':0.15},
        {'mode':'expert','leverage':3.0,'position_size':0.2,'buy_th':15,'sell_th':85,'position_ratio':0.4,'rebalance_threshold':0.15},
        {'mode':'expert','leverage':3.0,'position_size':0.25,'buy_th':12,'sell_th':88,'position_ratio':0.5,'rebalance_threshold':0.12},
        {'mode':'expert','leverage':5.0,'position_size':0.15,'buy_th':15,'sell_th':85,'position_ratio':0.3,'rebalance_threshold':0.15},
        {'mode':'expert','leverage':5.0,'position_size':0.2,'buy_th':12,'sell_th':88,'position_ratio':0.4,'rebalance_threshold':0.12},
    ]
...
""")
    ...
    with open('/tmp/backtest_v53.json', 'w') as f:
