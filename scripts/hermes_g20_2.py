...
def main():
    ...
    for coin, params in G19_PARAMS.items():
        ...
        signals.append({
            ...
            'sl': dyn_sl,
            'trend': trend,
            'trailing_stop': True  # G20.2独有
        })
        
        emoji = '🟢' if signal == 'BUY' else '🔴' if signal == 'SELL' else '🟡'
        trail = "追踪止损" if trend > 2 else ""
        print(f"  {emoji} {coin}: RSI={rsi:.1f}, TP={dyn_tp*100:.1f}%, SL={dyn_sl*100:.1f}%, 趋势={trend:.2f}% {trail}")
    
    with open('/home/goose/.openclaw/workspace/logs/g20_2_signals.json', 'w') as f:
