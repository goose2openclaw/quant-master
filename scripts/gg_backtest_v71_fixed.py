...
def simulate_fixed(initial_capital, price_data, cfg):
    ...
    for day_idx in range(min_days):
        ...
        for c in valid_coins:
            ...
            prices_list = [price_data[c][i]['close'] for i in range(max(0, day_idx-14), day_idx+1)]
            
            rsi = get_rsi(prices_list, cfg['rsi_period'])
...
print("="*70)
print("GO2SE Genius v7.1 修复版 - 30天回测")
print("="*70)
...
configs = [
    {'name':'普通-v71','mode':'normal','rsi_period':7,'rsi_buy':40,'rsi_sell':60,'bb_buy':25,'bb_sell':80,'position_ratio':0.65,'take_profit':0.08,'stop_loss':0.04},
    {'name':'普通-保守','mode':'normal','rsi_period':14,'rsi_buy':35,'rsi_sell':65,'bb_buy':25,'bb_sell':80,'position_ratio':0.5,'take_profit':0.10,'stop_loss':0.05},
    {'name':'普通-激进','mode':'normal','rsi_period':7,'rsi_buy':45,'rsi_sell':55,'bb_buy':30,'bb_sell':75,'position_ratio':0.7,'take_profit':0.06,'stop_loss':0.03},
    {'name':'专家-v71','mode':'expert','rsi_period':7,'rsi_buy':40,'rsi_sell':60,'bb_buy':25,'bb_sell':80,'position_ratio':0.65,'take_profit':0.08,'stop_loss':0.04},
    {'name':'专家-保守','mode':'expert','rsi_period':14,'rsi_buy':35,'rsi_sell':65,'bb_buy':25,'bb_sell':80,'position_ratio':0.5,'take_profit':0.10,'stop_loss':0.05},
    {'name':'专家-激进','mode':'expert','rsi_period':7,'rsi_buy':45,'rsi_sell':55,'bb_buy':30,'bb_sell':75,'position_ratio':0.7,'take_profit':0.06,'stop_loss':0.03},
]
...
expert = [r for r in results if r['mode']=='expert']

print(f"\n{'配置':14} {'RSI':8} {'仓位':6} {'止盈':6} {'止损':6} {'收益':10} {'胜率':7} {'交易':6}")
print("-"*70)
for r in normal:
    print(f"{r['name']:14} {r['rsi_buy']}/{r['rsi_sell']:5} {r['position_ratio']*100:5.0f}% {r['take_profit']*100:5.0f}% {r['stop_loss']*100:5.1f}% {r['return']:>+9.2f}% {r['win_rate']:>5.1f}% {r['total_trades']:5d}")
...
best_normal = normal[0] if normal else None
best_expert = expert[0] if expert else None

print("\n" + "="*70)
print("【最优配置】")
print("="*70)
if best_normal:
    print(f"普通模式: {best_normal['name']} 收益:{best_normal['return']:+.2f}% 胜率:{best_normal['win_rate']:.1f}%")
if best_expert:
    print(f"专家模式: {best_expert['name']} 收益:{best_expert['return']:+.2f}% 胜率:{best_expert['win_rate']:.1f}%")

with open('/tmp/backtest_v71_fixed.json', 'w') as f:
