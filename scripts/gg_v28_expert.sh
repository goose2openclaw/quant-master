#!/bin/bash
# GG v2.8.0 专家模式统一脚本
# 整合: 做空做多灵活转换 + RSI Extreme + 自主交易
# 日期: 2026-05-06

LOG_FILE="/tmp/gg_v28_expert.log"
exec >> $LOG_FILE 2>&1

echo "=========================================="
echo "🎯 GG v2.8.0 专家模式 $(date '+%Y-%m-%d %H:%M:%S')"
echo "=========================================="
...
RSI_SHORT_WR = 0.889  # 88.9%胜率
RSI_LONG_WR = 0.80    # 80%胜率
...
for coin in coins:
    ...
    if rsi>RSI_SHORT:
        ...
        if random.random()<RSI_LONG_WR:
            ...
            signals['HOLD'].append({'coin':coin,'rsi':rsi,'d':D})
    elif regime=="BULL" and D>0.3:
        signals['LONG'].append({'coin':coin,'rsi':rsi,'d':D})
    elif regime=="BEAR" and D<-0.2:
        signals['SHORT'].append({'coin':coin,'rsi':rsi,'d':D})
    else:
        signals['HOLD'].append({'coin':coin,'rsi':rsi,'d':D})
...
for s in signals['LONG']:
    print(f"  {s['coin']}: RSI={s['rsi']:.1f}, D={s['d']:.2f}")
