#!/usr/bin/env python3
"""
Hermès G12 v3 - 修复版模拟器
正确计算: 杠杆只影响盈亏计算,不影响本金
"""
...
def simulate_v3(initial_capital, price_data, cfg):
    """
    v3修复版:
    - 买入: 用本金的position_ratio买入
    - 杠杆: 只放大盈亏,不影响本金占用
    - 做空: 借入卖出,平仓买入
    """
        ...
        for c in valid_coins:
            ...
            if buy and capital > 10 and positions[c] == 0 and short_qtys[c] == 0:
                ...
                if total_cost <= capital:
                    ...
                    trades.append({'type':'BUY','coin':c,'price':d['close'],'qty':qty,'cost':total_cost})
            
            # ========== 卖出/止盈止损 ==========
            if positions[c] > 0:
                # 盈亏 = (当前价 - 入股价) / 入股价 * 杠杆
                pnl_ratio = (d['close'] - entry_prices[c]) / entry_prices[c] * leverage
                pnl = positions[c] * entry_prices[c] * pnl_ratio
                ...
                if sell:
                    revenue = positions[c] * d['close'] * (1 - 0.001)
                    capital += revenue + pnl
                    positions[c] = 0
                    entry_prices[c] = 0
                    trades.append({'type':'SELL','coin':c,'pnl_ratio':pnl_ratio,'pnl':pnl})
            ...
            if short_qtys[c] == 0 and positions[c] == 0:
                ...
                if short and capital > 20:
                    # 做空: 借入保证金*仓位的币,卖出
                    collateral = capital * position_ratio * 0.5
                    short_qty = collateral / d['close']
                    proceeds = short_qty * d['close']
                    fees = proceeds * 0.001
                    if fees < capital * 0.1:  # 手续费 < 本金10%
                        capital -= fees
                        short_qtys[c] = short_qty
                        short_entries[c] = d['close']
                        trades.append({'type':'SHORT_OPEN','coin':c,'price':d['close'],'qty':short_qty})
            
            # ========== 做空平仓 ==========
            if short_qtys[c] > 0:
                pnl_ratio = (short_entries[c] - d['close']) / short_entries[c] * leverage
                pnl = short_qtys[c] * short_entries[c] * pnl_ratio
                ...
                if cover:
                    buy_back = short_qtys[c] * d['close'] * (1 + 0.001)
                    capital += buy_back + pnl
                    short_qtys[c] = 0
                    short_entries[c] = 0
                    trades.append({'type':'SHORT_CLOSE','coin':c,'pnl_ratio':pnl_ratio,'pnl':pnl})
...
# v3 配置
configs = [
    {'name':'v3-1x普通','mode':'normal','leverage':1,'position':0.5,'rsi_buy':35,'rsi_sell':65,'bb_buy':25,'bb_sell':80,'short_rsi':70,'short_bb':85,'take_profit':0.10,'stop_loss':0.05,'rsi_period':7},
    {'name':'v3-2x普通','mode':'normal','leverage':2,'position':0.4,'rsi_buy':35,'rsi_sell':65,'bb_buy':25,'bb_sell':80,'short_rsi':70,'short_bb':85,'take_profit':0.08,'stop_loss':0.04,'rsi_period':7},
    {'name':'v3-3x普通','mode':'normal','leverage':3,'position':0.3,'rsi_buy':35,'rsi_sell':65,'bb_buy':25,'bb_sell':80,'short_rsi':70,'short_bb':85,'take_profit':0.06,'stop_loss':0.03,'rsi_period':7},
    {'name':'v3-1x专家','mode':'expert','leverage':1,'position':0.5,'rsi_buy':35,'rsi_sell':65,'bb_buy':25,'bb_sell':80,'short_rsi':70,'short_bb':85,'take_profit':0.10,'stop_loss':0.05,'rsi_period':7},
    {'name':'v3-2x专家','mode':'expert','leverage':2,'position':0.4,'rsi_buy':35,'rsi_sell':65,'bb_buy':25,'bb_sell':80,'short_rsi':70,'short_bb':85,'take_profit':0.08,'stop_loss':0.04,'rsi_period':7},
    {'name':'v3-3x专家','mode':'expert','leverage':3,'position':0.3,'rsi_buy':35,'rsi_sell':65,'bb_buy':25,'bb_sell':80,'short_rsi':70,'short_bb':85,'take_profit':0.06,'stop_loss':0.03,'rsi_period':7},
    {'name':'v3-做空2x','mode':'normal','leverage':2,'position':0.4,'rsi_buy':35,'rsi_sell':65,'bb_buy':25,'bb_sell':80,'short_rsi':60,'short_bb':75,'take_profit':0.08,'stop_loss':0.04,'rsi_period':7},
    {'name':'v3-高频2x','mode':'normal','leverage':2,'position':0.4,'rsi_buy':40,'rsi_sell':60,'bb_buy':30,'bb_sell':75,'short_rsi':65,'short_bb':80,'take_profit':0.05,'stop_loss':0.025,'rsi_period':5},
]

results = []
print(f"\n【执行{len(configs)}种配置回测】")
for cfg in configs:
    stats = simulate_v3(1000, price_data, cfg)
    if stats:
        result = {**cfg, **stats}
...
with open('/tmp/g12_v3_results.json', 'w') as f:
