...
def simulate_v8(initial_capital, price_data, cfg):
    ...
    for day_idx in range(min_days):
        ...
        for c in valid_coins:
            ...
            if cfg.get('mode') == 'expert':
                buy = (rsi < cfg.get('rsi_buy', 30) and bb_pos < cfg.get('bb_buy', 20)) and decision > cfg.get('decision', 0.7)
            else:
                buy = (rsi < cfg.get('rsi_buy', 30) or bb_pos < cfg.get('bb_buy', 20)) and decision > cfg.get('decision', 0.7)
...
print("="*70)
print("Hermes G12 v8 - 杠杆优化版 (3-5x)")
print("目标: 收益400%+")
print("="*70)
...
configs = [
    # 3x杠杆
    {'name':'3x-专家','mode':'expert','leverage':3,'position':0.30,'rsi_buy':30,'rsi_sell':70,'bb_buy':20,'bb_sell':80,'short_rsi':70,'short_bb':85,'take_profit':0.08,'stop_loss':0.04,'decision':0.70},
    {'name':'3x-保守','mode':'expert','leverage':3,'position':0.25,'rsi_buy':25,'rsi_sell':75,'bb_buy':15,'bb_sell':85,'short_rsi':65,'short_bb':80,'take_profit':0.10,'stop_loss':0.05,'decision':0.75},
    {'name':'3x-激进','mode':'expert','leverage':3,'position':0.40,'rsi_buy':30,'rsi_sell':70,'bb_buy':20,'bb_sell':80,'short_rsi':70,'short_bb':85,'take_profit':0.06,'stop_loss':0.03,'decision':0.65},
    # 4x杠杆
    {'name':'4x-专家','mode':'expert','leverage':4,'position':0.25,'rsi_buy':30,'rsi_sell':70,'bb_buy':20,'bb_sell':80,'short_rsi':70,'short_bb':85,'take_profit':0.08,'stop_loss':0.04,'decision':0.70},
    {'name':'4x-保守','mode':'expert','leverage':4,'position':0.20,'rsi_buy':25,'rsi_sell':75,'bb_buy':15,'bb_sell':85,'short_rsi':65,'short_bb':80,'take_profit':0.10,'stop_loss':0.05,'decision':0.75},
    {'name':'4x-激进','mode':'expert','leverage':4,'position':0.30,'rsi_buy':30,'rsi_sell':70,'bb_buy':20,'bb_sell':80,'short_rsi':70,'short_bb':85,'take_profit':0.06,'stop_loss':0.03,'decision':0.65},
    # 5x杠杆
    {'name':'5x-专家','mode':'expert','leverage':5,'position':0.20,'rsi_buy':30,'rsi_sell':70,'bb_buy':20,'bb_sell':80,'short_rsi':70,'short_bb':85,'take_profit':0.08,'stop_loss':0.04,'decision':0.70},
    {'name':'5x-保守','mode':'expert','leverage':5,'position':0.15,'rsi_buy':25,'rsi_sell':75,'bb_buy':15,'bb_sell':85,'short_rsi':65,'short_bb':80,'take_profit':0.10,'stop_loss':0.05,'decision':0.75},
    {'name':'5x-激进','mode':'expert','leverage':5,'position':0.25,'rsi_buy':30,'rsi_sell':70,'bb_buy':20,'bb_sell':80,'short_rsi':70,'short_bb':85,'take_profit':0.06,'stop_loss':0.03,'decision':0.65},
    # 普通模式
    {'name':'3x-普通','mode':'normal','leverage':3,'position':0.30,'rsi_buy':30,'rsi_sell':70,'bb_buy':20,'bb_sell':80,'short_rsi':70,'short_bb':85,'take_profit':0.08,'stop_loss':0.04,'decision':0.65},
    {'name':'4x-普通','mode':'normal','leverage':4,'position':0.25,'rsi_buy':30,'rsi_sell':70,'bb_buy':20,'bb_sell':80,'short_rsi':70,'short_bb':85,'take_profit':0.08,'stop_loss':0.04,'decision':0.65},
    {'name':'5x-普通','mode':'normal','leverage':5,'position':0.20,'rsi_buy':30,'rsi_sell':70,'bb_buy':20,'bb_sell':80,'short_rsi':70,'short_bb':85,'take_profit':0.08,'stop_loss':0.04,'decision':0.65},
]

results = []
print(f"\n执行{len(configs)}种配置回测")
for cfg in configs:
    stats = simulate_v8(1000, price_data, cfg)
    if stats:
        result = {**cfg, **stats}
        results.append(result)
        mark = "⭐" if stats['return'] > 100 else ""
        print(f"  {cfg['name']:12} 仓:{cfg['position']*100:.0f}% → 收益:{stats['return']:>+9.2f}% 胜率:{stats['win_rate']:>5.1f}% {mark}")
...
print(f"\n{'配置':12} {'杠杆':5} {'仓位':6} {'收益':10} {'胜率':7} {'交易':6}")
print("-"*55)
for r in results:
    print(f"{r['name']:12} {r['leverage']:5}x {r['position']*100:5.0f}% {r['return']:>+9.2f}% {r['win_rate']:>5.1f}% {r['trades']:>5d}")
...
with open('/tmp/g12_v8_results.json', 'w') as f:
