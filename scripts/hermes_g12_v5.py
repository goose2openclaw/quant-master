...
def simulate_v5(initial_capital, price_data, cfg):
    ...
    for day_idx in range(min_days):
        ...
        for c in valid_coins:
            ...
            if buy and capital > 10 and positions[c] == 0 and short_qtys[c] == 0:
                ...
                trades.append({'type':'LONG_OPEN','coin':c,'price':d['close'],'qty':qty})
            ...
            if positions[c] > 0:
                ...
                if sell:
                    ...
                    positions[c] = 0
                    entry_prices[c] = 0
                    trades.append({'type':'LONG_CLOSE','coin':c,'pnl':pnl})
            ...
            if short_qtys[c] == 0 and positions[c] == 0:
                ...
                if short and capital > 20:
                    ...
                    trades.append({'type':'SHORT_OPEN','coin':c,'price':d['close'],'qty':qty})
...
configs = [
    {'name':'v5-1x普通','mode':'normal','leverage':1,'position':0.5,'rsi_buy':35,'rsi_sell':65,'bb_buy':25,'bb_sell':80,'short_rsi':70,'short_bb':85,'take_profit':0.10,'stop_loss':0.05,'rsi_period':7},
    {'name':'v5-2x普通','mode':'normal','leverage':2,'position':0.4,'rsi_buy':35,'rsi_sell':65,'bb_buy':25,'bb_sell':80,'short_rsi':70,'short_bb':85,'take_profit':0.08,'stop_loss':0.04,'rsi_period':7},
    {'name':'v5-3x普通','mode':'normal','leverage':3,'position':0.3,'rsi_buy':35,'rsi_sell':65,'bb_buy':25,'bb_sell':80,'short_rsi':70,'short_bb':85,'take_profit':0.06,'stop_loss':0.03,'rsi_period':7},
    {'name':'v5-1x专家','mode':'expert','leverage':1,'position':0.5,'rsi_buy':35,'rsi_sell':65,'bb_buy':25,'bb_sell':80,'short_rsi':70,'short_bb':85,'take_profit':0.10,'stop_loss':0.05,'rsi_period':7},
    {'name':'v5-2x专家','mode':'expert','leverage':2,'position':0.4,'rsi_buy':35,'rsi_sell':65,'bb_buy':25,'bb_sell':80,'short_rsi':70,'short_bb':85,'take_profit':0.08,'stop_loss':0.04,'rsi_period':7},
    {'name':'v5-3x专家','mode':'expert','leverage':3,'position':0.3,'rsi_buy':35,'rsi_sell':65,'bb_buy':25,'bb_sell':80,'short_rsi':70,'short_bb':85,'take_profit':0.06,'stop_loss':0.03,'rsi_period':7},
    {'name':'v5-做空王','mode':'normal','leverage':2,'position':0.4,'rsi_buy':35,'rsi_sell':65,'bb_buy':25,'bb_sell':80,'short_rsi':60,'short_bb':75,'take_profit':0.08,'stop_loss':0.04,'rsi_period':7},
    {'name':'v5-高频2x','mode':'normal','leverage':2,'position':0.4,'rsi_buy':40,'rsi_sell':60,'bb_buy':30,'bb_sell':75,'short_rsi':65,'short_bb':80,'take_profit':0.05,'stop_loss':0.025,'rsi_period':5},
]

results = []
print(f"\n【执行{len(configs)}种配置回测】")
for cfg in configs:
    stats = simulate_v5(1000, price_data, cfg)
    if stats:
        result = {**cfg, **stats}
...
with open('/tmp/g12_v5_results.json', 'w') as f:
