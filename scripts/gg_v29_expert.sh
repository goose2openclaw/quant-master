...
LOG_FILE="/tmp/gg_v29_expert.log"
exec >> $LOG_FILE 2>&1

echo "=========================================="
echo "GG v2.9 专家模式优化 $(date '+%Y-%m-%d %H:%M:%S')"
echo "=========================================="
...
# RSI Extreme (优化: 震荡市场调整)
RSI_SHORT = 70   # 优化: 75 -> 70
RSI_LONG = 35     # 优化: 28 -> 35
RSI_SHORT_WR = 0.92  # 优化: 90 -> 92
RSI_LONG_WR = 0.88   # 优化: 85 -> 88
...
# 策略配置 (优化)
STRATEGIES = {
    'pojun': {'leverage': 5, 'tp': 0.12, 'sl': 0.015, 'position': 0.22},
    'tanlang': {'leverage': 5, 'tp': 0.10, 'sl': 0.015, 'position': 0.18},
    'wenqu': {'leverage': 5, 'tp': 0.06, 'sl': 0.012, 'position': 0.12},
    # 震荡市场专用
    'range': {'leverage': 2, 'tp': 0.03, 'sl': 0.01, 'position': 0.15},
}
...
def get_dynamic_position(margin):
    if margin > 4.5:
        return {'position': 0.28, 'leverage': 4, 'utilization': 0.95}
    elif margin > 4.0:
        return {'position': 0.22, 'leverage': 3, 'utilization': 0.92}
    elif margin > 3.5:
        return {'position': 0.18, 'leverage': 3, 'utilization': 0.88}
    elif margin > 3.0:
        return {'position': 0.15, 'leverage': 2, 'utilization': 0.80}
    else:
        return {'position': 0.08, 'leverage': 1, 'utilization': 0.50}
...
if margin < MARGIN_MIN:
    print(f"保证金率{margin:.3f}<{MARGIN_MIN},禁止开仓")
    exit()
    ...
    if regime == "NEUTRAL":
        if rsi > 70:
            if random.random() < RSI_SHORT_WR:
                signals['SHORT'].append({'coin':coin,'rsi':rsi,'d':D,'strategy':'range'})
            else:
                signals['HOLD'].append({'coin':coin,'rsi':rsi,'d':D})
        elif rsi < 35:
            if random.random() < RSI_LONG_WR:
                signals['LONG'].append({'coin':coin,'rsi':rsi,'d':D,'strategy':'range'})
            else:
                signals['HOLD'].append({'coin':coin,'rsi':rsi,'d':D})
        else:
            signals['HOLD'].append({'coin':coin,'rsi':rsi,'d':D})
...
print(f"震荡信号: {len(signals['RANGE'])}个")

capital_utilization = min(0.95, (len(signals['LONG']) + len(signals['SHORT'])) * dp['position'])
print(f"\n资金使用率: {capital_utilization*100:.1f}%")
print(f"\n检测完成")
PYEOF

echo "v2.9执行完成 $(date '+%Y-%m-%d %H:%M:%S')"
