...
def backtest(params, data, min_days=30, bb_threshold=0.25):
    cfg = params
    ...
    for d in range(min_days, min_d):
        for c in valid:
            ...
            if pos[c] == 0 and ratio < bb_threshold:
...
# 基于+27%优化的配置
OPTIMIZED = {
    'B+优化1': {'rsi_buy': 25, 'rsi_sell': 75, 'bb_buy': 20, 'bb_sell': 80, 'tp': 0.18, 'sl': 0.05, 'pos': 0.45, 'lev': 8},
    'B+优化2': {'rsi_buy': 25, 'rsi_sell': 75, 'bb_buy': 22, 'bb_sell': 78, 'tp': 0.20, 'sl': 0.05, 'pos': 0.50, 'lev': 8},
    'B+优化3': {'rsi_buy': 28, 'rsi_sell': 72, 'bb_buy': 20, 'bb_sell': 80, 'tp': 0.18, 'sl': 0.05, 'pos': 0.45, 'lev': 8},
    'B+优化4': {'rsi_buy': 25, 'rsi_sell': 75, 'bb_buy': 18, 'bb_sell': 82, 'tp': 0.18, 'sl': 0.06, 'pos': 0.45, 'lev': 8},
    'B+优化5': {'rsi_buy': 25, 'rsi_sell': 75, 'bb_buy': 20, 'bb_sell': 80, 'tp': 0.18, 'sl': 0.04, 'pos': 0.50, 'lev': 7},
}
...
for name, cfg in OPTIMIZED.items():
    r = backtest(cfg, data, min_days=30, bb_threshold=0.25)
