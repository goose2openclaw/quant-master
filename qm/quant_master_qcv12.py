...
class AutonomousDecision:
    ...
    def decide(self, symbol: str, combined: float, v8: float, g12: float, miro: float, rsi: float = 50) -> Dict:
        # 激进阈值: BUY >= 55, SELL <= 45
        # 但RSI<30或RSI>70时禁止操作（超卖/超买区域）
        
        # RSI保护机制
        if rsi < 30:
            d = {'action': 'HOLD', 'auto': False, 'reason': f"⛔ RSI超卖({rsi:.0f}) 禁止BUY"}
            print(f"   ⚠️ [风控] {symbol} RSI={rsi:.0f} 超卖, 禁止BUY")
        elif rsi > 75:
            d = {'action': 'HOLD', 'auto': False, 'reason': f"⛔ RSI超买({rsi:.0f}) 禁止SELL"}
            print(f"   ⚠️ [风控] {symbol} RSI={rsi:.0f} 超买, 禁止SELL")
        elif combined >= 55:
            d = {'action': 'BUY', 'auto': True, 'reason': f"✅ BUY信号 综合={combined:.0f} RSI={rsi:.0f}"}
            self.approved += 1
        elif combined <= 45:
            d = {'action': 'SELL', 'auto': True, 'reason': f"📤 SELL信号 综合={combined:.0f} RSI={rsi:.0f}"}
            self.approved += 1
        else:
            d = {'action': 'HOLD', 'auto': False, 'reason': f"⏸️ 观望 综合={combined:.0f} RSI={rsi:.0f}"}
...
# ============================================================
# SelfCorrectionEngine - 自动风控纠正
# ============================================================
class SelfCorrectionEngine:
    def __init__(self):
        self.concentration_limit = 0.40  # 单币种占比上限40%
        self.rsi_take_profit = 65  # RSI>65部分止盈
        self.rsi_stop_loss = 20   # RSI<20止损
        self.rebalance_threshold = 0.50  # 占比超过50%触发再平衡
    
    def check_concentration(self, symbol: str, balance: float, total_value: float, price: float) -> Dict:
        """检查持仓集中度"""
        if total_value <= 0: return {'action': 'NONE'}
        ...
        if concentration > self.concentration_limit:
            # 计算需要卖出的数量
            target_pct = self.concentration_limit * 0.8  # 目标降到40%以下
            ...
            return {
                ...
                'symbol': symbol,
                'quantity': sell_qty,
                'reason': f"集中度{concentration*100:.1f}%>40%, 减持至{target_pct*100:.0f}%",
                'concentration': concentration
            }
        
        return {'action': 'NONE'}
    
    def check_rsi_take_profit(self, symbol: str, rsi: float, entry_price: float, current_price: float) -> Dict:
        """RSI超买止盈检查"""
        if rsi > self.rsi_take_profit:
            pnl_pct = (current_price - entry_price) / entry_price * 100
            if pnl_pct > 5:  # 盈利超过5%才止盈
                return {
                    'action': 'TAKE_PROFIT',
                    'type': 'SELL',
                    'symbol': symbol,
                    'quantity': 0,  # 全部或按配置
                    'reason': f"RSI={rsi:.0f}>{self.rsi_take_profit}超买, 盈亏{pnl_pct:.1f}%止盈",
                    'rsi': rsi
                }
        
        return {'action': 'NONE'}
    
    def check_rsi_stop_loss(self, symbol: str, rsi: float, entry_price: float, current_price: float) -> Dict:
        """RSI超卖止损检查"""
        if rsi < self.rsi_stop_loss:
            ...
            return {
                ...
                'symbol': symbol,
                'quantity': 0,
                'reason': f"RSI={rsi:.0f}<{self.rsi_stop_loss}超卖止损, 盈亏{pnl_pct:.1f}%",
                'rsi': rsi,
                'pnl_pct': pnl_pct
            }
    ...
    def auto_correct(self, holdings: Dict, prices: Dict, total_value: float) -> List[Dict]:
        """自动风控纠正"""
        actions = []
        
        for symbol, data in holdings.items():
            if data.get('balance', 0) <= 0: continue
            
            price = prices.get(symbol, 0)
            if price <= 0: continue
            
            # 1. 集中度检查
            conc = self.check_concentration(
                symbol, data['balance'], total_value, price
            )
            if conc['action'] != 'NONE':
                actions.append(conc)
            ...
            if data.get('entry_price'):
                take = self.check_rsi_take_profit(
                    symbol, data.get('rsi', 50),
...
class QuantMasterQCV12:
    ...
    def run_cycle(self) -> Dict:
        ...
        print(f"💰 USDT: ${self.binance.balance.get('USDT', 0):.2f} | ADA: {self.binance.balance.get('ADA', 0):.2f}")
        print(f"{'='*70}")
        ...
        for sym in symbols:
            ...
            d = self.autonomous.decide(sym, combined, v8_s, g12_s, miro_s, rsi=v8.get('rsi', 50))
        ...
        # 5. Self-Correction 自动风控
        print(f"\n[5] 🔧 自动风控检查...")
        # Build holdings dict - use symbol without USDT suffix
        holdings = {}
        price_map = {}
        for r in results:
            sym = r['symbol'].replace('USDT', '')
            holdings[sym] = {
                'balance': self.binance.balance.get(sym, 0),
                'entry_price': 0,
                'rsi': r.get('rsi', 50) if isinstance(r.get('v8'), dict) else 50
            }
            price_map[sym] = r['price']
        holdings['USDT'] = {'balance': self.binance.balance.get('USDT', 0), 'entry_price': 0, 'rsi': 50}
        price_map['USDT'] = 1
        
        # Calculate total value
        total_val = sum(h.get('balance', 0) * price_map.get(sym, 1) for sym, h in holdings.items())
        
        corrections = self.self_correct.auto_correct(holdings, price_map, total_val)
        for corr in corrections:
            print(f"   🔴 [自动纠正] {corr['reason']}")
            if corr['action'] == 'REBALANCE':
                symbol = corr['symbol'] + 'USDT'
                qty = corr['quantity']
                if qty > 0.01:  # Minimum quantity check
                    print(f"   📤 执行减持: 卖出 {qty:.2f} {symbol}")
                    self.binance.place_order(symbol, 'SELL', qty)
            if corr['action'] == 'REBALANCE' and corr['quantity'] > 0:
                # 执行减持
                print(f"   📤 执行: 卖出 {corr['quantity']:.4f} {corr['symbol']}")
        ...
        print(f"\n[6] 💓 Watchdog: {health['health']} ({health['responsiveness']:.0f}%)")
